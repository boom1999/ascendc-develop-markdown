...
![](images/atlas_ascendc_best_practices_10_0040_img_011.png)

3. 计算ds，本计算中，FlashSoftmaxGrad计算的入参为dy、正向输出attention_in，该结果与dp做减操作，最终的结果与p相乘得到结果ds。

![](images/atlas_ascendc_best_practices_10_0040_img_012.png)

4. 计算dq，本计算将ds结果与key做matmul计算，并将结果与scale相乘得到结果dq。

![](images/atlas_ascendc_best_practices_10_0040_img_013.png)

5. 计算dk，本计算将转置后的ds结果与query做matmul计算，并将结果与scale相乘得到结果dk。

![](images/atlas_ascendc_best_practices_10_0040_img_014.png)

6. 计算dv，本计算将p的结果做drop计算，转置后与dy做matmul计算。

![](images/atlas_ascendc_best_practices_10_0040_img_015.png)
...
![](images/atlas_ascendc_best_practices_10_0040_img_016.png)
...
- 观察如下流水图，绿色为Vector侧的流水，橙色为Cube侧的流水，可以看出两侧的流水都存在大段的空隙，CV之间流水很大程度上未并行，需要考虑CV流水优化。
      **图3 **优化前算子流水
![](images/atlas_ascendc_best_practices_10_0040_img_017.png)
...
![](images/atlas_ascendc_best_practices_10_0040_img_018.png)
- 场景一的Profiling数据如下，aic_fixpipe_ratio占比极高，可能存在FixPipe bound。
      **图5 **场景一Profiling数据
![](images/atlas_ascendc_best_practices_10_0040_img_019.png)
- 场景二的Profiling数据如下，mte2_ratio占比高，可能存在MTE2 bound。
      **图6 **场景二Profiling数据
![](images/atlas_ascendc_best_practices_10_0040_img_020.png)
...
**图7 **优化前计算基本块及次数
![](images/atlas_ascendc_best_practices_10_0040_img_021.png)

考虑到UB空间没有用满，基本块调整到(128, 128)，如下图优化后只需循环计算16次，切分后算子性能提升一倍。

**图8 **优化后计算基本块及次数
![](images/atlas_ascendc_best_practices_10_0040_img_022.png)
- 优化点二：CV流水并行
...
![](images/atlas_ascendc_best_practices_10_0040_img_023.png)

如上图是实现mm1、mm2和mm3缓存的流水图，并行度提高，CV的间隔减小，提升了算子性能。

**图10 **Vector等Cube流水的间隔插入Vector计算
![](images/atlas_ascendc_best_practices_10_0040_img_024.png)
...
![](images/atlas_ascendc_best_practices_10_0040_img_025.png)**图12 **causal场景优化后每个核计算量
![](images/atlas_ascendc_best_practices_10_0040_img_026.png)
...
![](images/atlas_ascendc_best_practices_10_0040_img_027.png)
...
![](images/atlas_ascendc_best_practices_10_0040_img_028.png)
- 优化点五：MTE2优化

结合如下的Profiling数据和流水图，可以看出MTE2 bound，且部分MTE2搬运时间异常。

**图15 **场景二Profiling数据
![](images/atlas_ascendc_best_practices_10_0040_img_029.png)**图16 **场景二流水图
![](images/atlas_ascendc_best_practices_10_0040_img_030.png)
