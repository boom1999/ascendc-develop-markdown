...
-   hifloat8\_t
    ...
    **表 4**  hifloat8\_t类型取值范围
    ...
    -   **符号位Sv**
        ...
        ![](figures/zh-cn_formulaimage_0000002523346906.png)
    ...
    -   **指数位Ev**
        ...
        **表 5**  不同D域值，Es，Em bit大小不同
        ...
        ![](figures/zh-cn_formulaimage_0000002554426831.png)

        Em计算公式如下，Emi表示Em每个bit位的值（0或1），D表示点域值，参考[表5](#table15755618132)。

        ![](figures/zh-cn_formulaimage_0000002523346916.png)

        Ev计算公式：

        ![](figures/zh-cn_formulaimage_0000002523306938.png)

        Mv计算公式，M表示位数值，bitwidth of M表示M的bit宽度大小，参考[表5](#table15755618132)。

        ![](figures/zh-cn_formulaimage_0000002523346920.png)

    -   在Normal和Subnormal模式下，浮点数的取值计算公式不同：

        Normal模式：由Sv、Ev、Mv组成。

        ![](figures/zh-cn_formulaimage_0000002523346912.png)

        Subnormal模式，由Sv、Mv组成。

        ![](figures/zh-cn_formulaimage_0000002554426813.png)

        ![](figures/zh-cn_formulaimage_0000002523346908.png)
...
-   half
    ...
    ![](figures/流水任务运行示意图-81.png)

    half特殊值bit位表示如下：

    ![](figures/hifi8特殊值-82.png)

-   bfloat16\_t

    下图是一个bfloat16\_t数据类型的示例，其符号位占用1位，指数占用8位，尾数占用7位。

    ![](figures/流水任务运行示意图-83.png)

    bfloat16\_t特殊值bit位表示如下：

    ![](figures/hifi8特殊值-84.png)

-   float

    下图是一个float类型的示例，其符号位占用1位，指数占用8位，尾数占用23位。

    ![](figures/流水任务运行示意图-85.png)

    float特殊值bit位表示如下：

    ![](figures/hifi8特殊值-86.png)
