...
![](figures/zh-cn_formulaimage_0000002523289428.png)

为方便表达，以变量S和P表示计算公式：

![](figures/zh-cn_formulaimage_0000002523129434.png)

![](figures/zh-cn_formulaimage_0000002554409339.png)

![](figures/zh-cn_formulaimage_0000002554409295.png)

则注意力的反向计算公式为：

![](figures/zh-cn_formulaimage_0000002523129454.png)

![](figures/zh-cn_formulaimage_0000002523289444.png)

![](figures/zh-cn_formulaimage_0000002523289422.png)

![](figures/zh-cn_formulaimage_0000002523129416.png)
...
1.  重计算p，本步骤重计算了FlashAttention流程中的softmax结果p，计算结果保存在ub中。
    ...
    ![](figures/zh-cn_formulaimage_0000002523129392.png)

2.  计算dp，该计算包含matmul计算和dropout计算，matmul计算中，左矩阵为dy，右矩阵为转置后的value。

    ![](figures/zh-cn_formulaimage_0000002554329361.png)

3.  计算ds，本计算中，FlashSoftmaxGrad计算的入参为dy、正向输出attention\_in，该结果与dp做减操作，最终的结果与p相乘得到结果ds。

    ![](figures/zh-cn_formulaimage_0000002554329359.png)

4.  计算dq，本计算将ds结果与key做matmul计算，并将结果与scale相乘得到结果dq。

    ![](figures/zh-cn_formulaimage_0000002554329369.png)

5.  计算dk，本计算将转置后的ds结果与query做matmul计算，并将结果与scale相乘得到结果dk。

    ![](figures/zh-cn_formulaimage_0000002554409287.png)

6.  计算dv，本计算将p的结果做drop计算，转置后与dy做matmul计算。

    ![](figures/zh-cn_formulaimage_0000002554329373.png)
...
-   对于上述场景一，causal场景下可能存在核间分布不均匀的情况，如下图经过atten\_mask掩码后，红色部分是算子需要计算的部分，绿色无需计算；如果不按照基本块的个数来分核，按照第一根轴的大小8（行）来分核，假设平均分到9个核上，每个核做ceil\(8 / 9\) = 1行，则第1个核只需做1个基本块，但是第8个核需要做8个基本块的计算，出现明显的负载不均衡。因此需要考虑将红色块均匀分到多个核上计算。
    ...
    **图 4**  causal场景atten\_mask形状<a name="fig13819650657"></a>  
    ![](figures/causal场景atten_mask形状-63.png "causal场景atten_mask形状-63")
...
-   优化点五：MTE2优化
    ...
    **图 15**  场景二Profiling数据<a name="fig932111118913"></a>  
    ![](figures/场景二Profiling数据-64.png "场景二Profiling数据-64")
