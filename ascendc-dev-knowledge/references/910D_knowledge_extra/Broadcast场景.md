# Broadcast场景<a name="ZH-CN_TOPIC_0000002554409007"></a>

在某些场景下，可能会存在两个输入shape不相同的情况。由于[Add](Add.md)接口只支持对shape相同的输入进行计算，因此需要先对输入进行shape变换，再进行Add计算。本节将对满足Broadcast条件的输入在算子实现中的Broadcast处理进行介绍，其他场景可以参考本章节中提供的思路。
...
本节中将使用[Broadcast](Broadcast.md)接口，因此输入需满足该API相关约束。同时，由于硬件限制，该API的输入地址需满足32字节对齐。本节以输入维度为2、第二个轴（axis = 1）需要Broadcast为例进行说明。完整的样例代码请参见[输入Broadcast的Add算子样例](https://gitee.com/ascend/samples/tree/master/operator/ascendc/0_introduction/21_vectoradd_kernellaunch/VectorAddMultiCoreWithTilingBroadcast)。
...
```
constexpr uint32_t BLOCK_SIZE = 32;
if (shorterAxisLen % (NUM_BLOCKS * BUFFER_NUM) == 0) {
    uint32_t blockLength = shorterAxisLen / NUM_BLOCKS * coef;
    ...
} else {
    uint32_t formerNum = (shorterAxisLen / BUFFER_NUM) % NUM_BLOCKS;
    uint32_t tailNum = NUM_BLOCKS - formerNum;

    uint32_t formerLength = ((shorterAxisLen / BUFFER_NUM) / NUM_BLOCKS + 1) * BUFFER_NUM * coef;
    uint32_t tailLength = ((shorterAxisLen / BUFFER_NUM) / NUM_BLOCKS) * BUFFER_NUM * coef;
    ....
}
```
...
由于数据是向coef对齐的，在数据拷贝的过程中可能会出现地址不满足32字节对齐的场景，因此CopyIn函数、CopyOut函数中使用[DataCopyPad](DataCopyPad(ISASI).md)进行数据拷贝。
